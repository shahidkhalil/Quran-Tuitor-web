...
import { getCheckoutContextFromTrial } from "@/server/actions/payments";
...
type Props = {
  searchParams: Promise<{ from_trial?: string; cancelled?: string }>;
};
...
export default async function ParentCheckoutPage({ searchParams }: Props) {
  ...
  const { from_trial: fromTrial, cancelled } = await searchParams;
  const ctx = fromTrial
    ...
    : {
        ...
        lessonCount: 4,
        error:
          "Open checkout from Bookings after your free trial (Continue to paid lessons).",
      };
  ...
  return (
    <div>
      ...
      {!ctx.error && ctx.trial && ctx.amountCents != null && ctx.rateUsd != null ? (
        <section className="surface-card space-y-5 p-5 md:p-6">
          <div>
            <p className="eyebrow text-[var(--color-accent)]">Package</p>
          ...
          {!stripeOk ? (
            ...
            <StartCheckoutButton trialId={ctx.trial.id} />
          )}
